...
class AccessLocationScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: SafeArea(child: BgWidget(child: Padding(
...
class BottomButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Center(child: SizedBox(width: 700, child: Column(children: [
      ...
      SizedBox(height: Get.find<AuthController>().isLoggedIn() ? Dimensions.PADDING_SIZE_SMALL : 0),
      ...
      Get.find<AuthController>().isLoggedIn() ? CustomButton(
        ...
        onPressed: () => Get.dialog(MogoCodeDialog(onGetAddress: (AddressModel address) async {
          Get.dialog(CustomLoader(), barrierDismissible: false);
          ZoneResponseModel _response = await locationController.getZone(address.latitude, address.longitude, false);
          if(_response.isSuccess) {
            locationController.saveAddressAndNavigate(address, fromSignUp, route, route != null);
          }else {
            Get.back();
            Get.toNamed(RouteHelper.getPickMapRoute(route == null ? RouteHelper.accessLocation : route, route != null));
            showCustomSnackBar('service_not_available_in_current_location'.tr);
